        ...
        <div className="h-full bg-[var(--bg-app)] p-4 @sm:p-6 overflow-auto">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="grid grid-cols-1 @sm:grid-cols-2 gap-4 @sm:p-6 mb-8">
                    <div className="bg-[var(--bg-surface)] p-4 @sm:p-6 rounded-2xl border border-[var(--border-primary)] shadow-xl">
                        <div className="text-[10px] font-black text-[var(--text-muted)] mb-1">Performance</div>
                        <div className="text-3xl font-bold text-[var(--text-primary)]">{analysis.avgDuration}ms</div>
                        <div className="text-xs text-[var(--text-muted)] mt-1">Average Response Time</div>
                    </div>
                    <div className="bg-[var(--bg-surface)] p-4 @sm:p-6 rounded-2xl border border-[var(--border-primary)] shadow-xl">
                        <div className="text-[10px] font-black text-[var(--text-muted)] mb-1">Scale</div>
                        <div className="text-3xl font-bold text-[var(--text-primary)]">{analysis.total}</div>
                        <div className="text-xs text-[var(--text-muted)] mt-1">Parallel Requests Processed</div>
                    </div>
                </div>

                <div className="bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-primary)] overflow-hidden">
                    <div className="px-4 @sm:px-6 py-4 border-b border-[var(--border-primary)] bg-[var(--bg-surface)]/50 flex justify-between items-center">
                        <h3 className="text-xs font-bold tracking-widest text-[var(--text-tertiary)]">Domain Distribution</h3>
                    </div>
