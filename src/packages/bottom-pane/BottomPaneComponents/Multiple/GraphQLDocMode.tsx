...
export const GraphQLDocMode = () => {
            ...
            <div className="h-full flex flex-col items-center justify-center text-zinc-500 bg-[var(--bg-app)] p-8 text-center relative overflow-hidden">
                ...
                <div className="relative z-10 animate-premium-fade-in max-w-md w-full">
                    <div className="bg-[var(--bg-surface)]/40 backdrop-blur-xl border border-[var(--border-primary)]/50 rounded-3xl p-10 shadow-2xl shadow-black/50">
                        ...
                        <h3 className="text-xl font-black text-[var(--text-primary)] mb-3 tracking-tight uppercase italic">GraphQL Documentation</h3>
    ...
    if (loading) {
        return (
            <div className="h-full flex flex-col items-center justify-center bg-[var(--bg-app)]">
                ...
                <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)]">Analyzing Operations...</div>
            </div>
        );
    }
    ...
    if (docs.length === 0) {
        return (
            <div className="h-full flex flex-col items-center justify-center text-zinc-500 bg-[var(--bg-app)] p-8 text-center">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-primary)] shrink-0 bg-[var(--bg-sidebar)]">
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)]">GraphQL Documentation</span>
                    </div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar p-6 bg-grid-pattern">
                <div className="max-w-4xl mx-auto space-y-4">
                    {docs.map((doc: any, idx) => {
                        ...
                        return (
                            <div key={id} className="bg-[var(--bg-surface)]/40 rounded-xl border border-[var(--border-primary)]/50 overflow-hidden backdrop-blur-sm group hover:border-pink-500/30 transition-all duration-300 shadow-xl shadow-black/20">
                                {/* Header (Clickable) */}
                                <div 
                                    onClick={() => toggleExpand(id)}
                                    className="flex items-center justify-between px-4 py-3 bg-[var(--bg-surface)]/60 border-b border-[var(--border-primary)]/20 cursor-pointer hover:bg-[var(--bg-surface-elevated)]/40 transition-colors"
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h4 className="text-xs font-bold text-[var(--text-primary)] font-mono tracking-tight">{doc.operationName}</h4>
                                    </div>
                                    ...
                                    <div className="p-4 space-y-4 animate-premium-fade-in">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-1.5 text-[10px] text-[var(--text-tertiary)] font-bold uppercase tracking-wider">
                                                    <FiCode className="text-pink-400" />
                                                    Operation
                                                </div>
                                                <button 
                                                    onClick={(e) => handleCopy(doc.query, id + '-q', e)}
                                                    className="p-1 hover:bg-[var(--bg-surface-elevated)] rounded transition-colors text-[var(--text-muted)] hover:text-[var(--text-primary)]"
                                                >
                                                    {copiedId === id + '-q' ? <FiCheck className="text-green-500" /> : <FiCopy size={12} />}
                                                </button>
                                            </div>
                                            <pre className="bg-[var(--bg-surface-inset)]/40 p-4 rounded-lg border border-[var(--border-primary)]/50 text-[11px] text-[var(--text-secondary)] font-mono leading-relaxed overflow-x-auto">
                                                {doc.query}
                                            </pre>
                                        </div>
                                        ...
                                        {doc.variables && doc.variables !== "{}" && (
                                            <div className="space-y-2">
                                                ...
                                                <pre className="bg-[var(--bg-surface-inset)]/40 p-4 rounded-lg border border-[var(--border-primary)]/50 text-[11px] text-[var(--text-tertiary)] font-mono leading-relaxed overflow-x-auto">
                                                    {doc.variables}
                                                </pre>
                                            </div>
                                        )}
