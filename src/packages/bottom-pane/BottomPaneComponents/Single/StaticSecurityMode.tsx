...
const FindingCard = ({ finding }: { finding: StaticAnalysisFinding & { isCustom?: boolean, isError?: boolean, scriptName?: string } }) => (
    <div className={twMerge(
        ...
        finding.isError
            ...
            : "border-[var(--border-primary)]/30 bg-[var(--bg-surface)]/5 hover:border-orange-500/50"
    )}>
        <div className="p-4 flex flex-col gap-3">
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-[var(--text-muted)] tracking-widest pl-1">Heuristic Result</span>
                </div>
                <div className={twMerge(
            ...
            <div className="px-6 pt-4 border-b border-[var(--border-primary)] bg-[var(--bg-sidebar)] flex flex-col shadow-lg shrink-0">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-black text-[var(--text-primary)] tracking-tighter">Fast Static Inspector</h2>
                            <div className="text-[9px] text-[var(--text-muted)] font-bold tracking-widest italic text-orange-600/70">Real-time Heuristic Pattern Matching</div>
                        </div>
                    </div>
                    ...
                    {(['Analysis', 'Custom Scripts'] as const).map(t => (
                        <button
                            ...
                            className={twMerge(
                                ...
                                tab === t ? "text-orange-500" : "text-[var(--text-muted)] hover:text-[var(--text-tertiary)]"
                            )}
                        >
                            {t}
                            {tab === t && (
                ...
                <div className="max-w-4xl mx-auto p-4 @sm:p-8 space-y-8">
                    {tab === 'Custom Scripts' ? (
                        ...
                        <div className="space-y-8">
                            ...
                            <div>
                                <div className="text-[10px] font-black text-[var(--text-muted)] tracking-widest pl-1 mb-4">Core Engine Diagnostics</div>
                                <div className="grid grid-cols-1 @sm:grid-cols-2 gap-4">
                                    {results.map((res, i) => (
                                        <div key={i} className="bg-[var(--bg-surface)]/40 border border-[var(--border-primary)] rounded-2xl p-5 flex flex-col gap-3 group hover:bg-[var(--bg-surface-elevated)]/60 transition-all duration-300 shadow-lg">
                                            <div className="flex justify-between items-start">
                                                <div className="text-xs font-black text-[var(--text-secondary)] italic tracking-tighter">{res.check}</div>
                                                <span className={twMerge(
                                            ...
                                            <div className="text-[11px] text-[var(--text-muted)] group-hover:text-[var(--text-tertiary)] transition-colors leading-relaxed italic">
                                                {res.message}
                                            </div>

                                            <div className="mt-auto pt-3 border-t border-[var(--border-primary)]/50 flex flex-col gap-1.5 font-sans">
                                                <span className="text-[8px] font-black text-[var(--text-muted)] tracking-widest opacity-60">Potential Impact</span>
                                                <span className="text-[10px] text-[var(--text-tertiary)]/80 font-medium italic leading-relaxed">{res.impact}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="bg-[var(--bg-surface-elevated)]/10 border border-[var(--border-primary)]/20 rounded-3xl p-6 relative overflow-hidden shadow-inner">
                                ...
                                <div className="grid grid-cols-3 gap-6">
                                    <div>
                                        <div className="text-[9px] font-bold text-[var(--text-muted)] mb-1">Execution Speed</div>
                                        <div className="text-xs font-black text-[var(--text-primary)] italic tracking-tighter">142ms <span className="text-emerald-500 text-[8px] opacity-60">OPTIMAL</span></div>
                                    </div>
                                    <div>
                                        <div className="text-[9px] font-bold text-[var(--text-muted)] mb-1">Signature Count</div>
                                        <div className="text-xs font-black text-[var(--text-primary)] italic tracking-tighter">1,240 <span className="text-orange-500 text-[8px] opacity-60 underline underline-offset-2">LATEST</span></div>
                                    </div>
                                    <div>
                                        <div className="text-[9px] font-bold text-[var(--text-muted)] mb-1">Confidence Score</div>
                                        <div className="text-xs font-black text-[var(--text-primary)] italic tracking-tighter">98.2% <span className="text-blue-500 text-[8px] opacity-60 italic">STABLE</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
...
const Placeholder = ({ text }: { text: string }) => (
    <div className="h-full flex flex-col items-center justify-center bg-[var(--bg-app)] p-6 @sm:p-10 text-center font-sans">
        ...
        <h3 className="text-[var(--text-tertiary)] font-black mb-2 italic tracking-tight">Static Inspector Standby</h3>
        <p className="text-[10px] text-[var(--text-muted)] max-w-[200px] leading-relaxed font-bold tracking-widest">{text}</p>
    </div>
);
