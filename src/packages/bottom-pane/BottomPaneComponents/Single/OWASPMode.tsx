    ...
    <div className="h-full bg-[var(--bg-app)] flex flex-col overflow-hidden">
      <div className="flex flex-col @sm:flex-row items-start @sm:items-center px-4 @sm:px-6 py-4 border-b border-[var(--border-primary)] bg-[var(--bg-sidebar)] justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-black text-[var(--text-primary)] tracking-tighter">OWASP Security Auditor</h2>
            <div className="text-[9px] text-[var(--text-muted)] font-bold tracking-widest">Top 10 Vulnerability Scanner</div>
          </div>
        </div>

        {analyzing && (
          <div className="flex items-center gap-2">
            <div className="w-12 h-1 bg-[var(--bg-surface-inset)] rounded-full overflow-hidden relative">
      ...
      <div className="flex-grow p-4 @sm:p-6 overflow-y-auto no-scrollbar pb-10">
        <div className="grid grid-cols-1 gap-4 max-w-4xl">
          {findings.map((finding) => (
            <div key={finding.id} className="group bg-[var(--bg-surface)]/40 border border-[var(--border-primary)] rounded-2xl p-5 hover:border-red-500/30 transition-all duration-300 relative overflow-hidden">
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-black bg-[var(--bg-surface-inset)] text-[var(--text-muted)] px-2 py-0.5 rounded">{finding.id}</span>
                ...
                <span className="text-[10px] font-bold text-[var(--text-muted)] tracking-widest">{finding.category}</span>
              </div>

              <h3 className="text-[var(--text-secondary)] font-bold mb-2 group-hover:text-[var(--text-primary)] transition-colors">{finding.title}</h3>
              <p className="text-xs text-[var(--text-muted)] leading-relaxed">{finding.description}</p>

              <div className="mt-4 pt-4 border-t border-[var(--border-primary)]/50 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="text-[9px] text-[var(--text-muted)] font-mono italic">Detected via Static Analysis Engine</div>
...
const Placeholder = ({ text }: { text: string }) => (
  <div className="h-full flex flex-col items-center justify-center bg-[var(--bg-app)] p-6 @sm:p-10 text-center">
    ...
    <h3 className="text-[var(--text-tertiary)] font-bold mb-1 italic">OWASP Audit Ready</h3>
